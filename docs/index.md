---
title: 首页
description: 程嘉骏的个人主页
---

<section class="hero-panel">
  <p class="eyebrow">人工智能专业应届毕业生</p>
  <h1>程嘉骏</h1>
  <p class="role">软件工程师 / 硬件工程师 / AI应用开发工程师</p>
  <p class="lead">
    你好，我是程嘉骏，一名人工智能专业本科应届毕业生。
    我关注AI应用开发、计算机视觉、智能Agent以及软硬件结合方向。
    目前正在寻找软件工程师、硬件工程师或AI应用开发相关岗位。
  </p>
  <div class="tag-row">
    <span>AI应用开发</span>
    <span>计算机视觉</span>
    <span>软件 / 硬件工程</span>
  </div>
  <div class="action-row">
    <a class="button primary" href="/projects/">查看项目</a>
    <a class="button" href="/resume.pdf" download>下载简历</a>
    <a class="button ghost" href="https://github.com/0110none" target="_blank" rel="noreferrer">访问GitHub</a>
  </div>
</section>

## 主页概览

<div class="feature-grid">
  <a class="feature-card" href="/projects/">
    <span class="card-kicker">Projects</span>
    <h3>项目展示</h3>
    <p>整理人脸识别系统、AI招聘顾问Agent和竞赛项目经历，突出实践过程和真实职责。</p>
  </a>
  <a class="feature-card" href="/skills">
    <span class="card-kicker">Skills</span>
    <h3>技术栈</h3>
    <p>按编程语言、AI视觉、Agent、数据库和开发工具分类展示目前掌握的技术。</p>
  </a>
  <a class="feature-card" href="/blog/">
    <span class="card-kicker">Blog</span>
    <h3>技术博客</h3>
    <p>后续记录毕业设计开发、InsightFace实践、AI辅助开发和个人作品集建设经验。</p>
  </a>
</div>

## 近期关注

<div class="mini-timeline">
  <div>
    <strong>AI应用开发</strong>
    <p>关注大模型工具链、智能Agent流程设计和实际业务场景落地。</p>
  </div>
  <div>
    <strong>计算机视觉</strong>
    <p>围绕人脸检测、特征提取、多路视频处理和桌面端系统集成做项目实践。</p>
  </div>
  <div>
    <strong>软硬件结合</strong>
    <p>学习嵌入式系统设计、PLC编程及应用，尝试把软件能力与硬件场景结合。</p>
  </div>
</div>
